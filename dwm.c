...
static void deccnmaster(const Arg *arg);
...
void deccnmaster(const Arg *arg) {
  selmon->nmaster = MAX(selmon->nmaster + arg->i, 0);
  arrange(selmon);
}
...
void setgaps(const Arg *arg) {
  ...
  // Count the number of visible clients in the current tag (workspace)
  for (c = selmon->clients; c; c = c->next) {
    if (ISVISIBLE(c)) // Only count clients visible on the current tag
      client_count++;
  }

  // Enable or disable gaps based on the number of visible clients
  if (client_count > 1) {
    selmon->drawwithgaps = 1; // Enable gaps if there are multiple clients
    selmon->gappx = gappx;    // Set gap size to default
  } else {
    selmon->drawwithgaps = 0; // Disable gaps if only one client is visible
    selmon->gappx = 0;        // Set gap size to zero when gaps are disabled
  }

  // Apply gap adjustments based on the argument
  switch (arg->i) {
  case GAP_TOGGLE:
    selmon->drawwithgaps = !selmon->drawwithgaps;
    selmon->gappx = selmon->drawwithgaps ? gappx : 0;
    break;
  case GAP_RESET:
    selmon->gappx = gappx;
    break;
  ...
  // Rearrange windows on the monitor with the updated gap settings
  arrange(selmon);
}
...
void tile(Monitor *m) {
  ...
  // Automatically enable or disable gaps based on the number of clients
  m->drawwithgaps = (n > 1);

  if (m->drawwithgaps) { /* draw with full gaps logic */
    if (n > m->nmaster)
